...
```{r}
gbna2 <- st_read(dsn = "data/geospatial", 
                layer = "geoBoundaries-NGA-ADM2")
```

```{r}
gbna2_sim <- st_read(dsn = "data/geospatial", 
                    layer = "geoBoundaries-NGA-ADM2_simplified")
```

Seems `gbna2` is same as `gbna2_sim`.

### Importing Attribute Data into R

Next, we will import eqje-vguj.csv file into RStudio and save the file into an R dataframe called *eqje*.

The task will be performed by using *read_csv()* function of **readr** package as shown in the code chunk below.

```{r}
eqje <- read_csv("data/aspatial/eqje-vguj.csv")
```
