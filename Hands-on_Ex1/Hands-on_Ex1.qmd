...
> Note: The purpose of this section is not merely extracting the necessary data sets. It also aims to introduce you to public available data sets.
...
The **layer** should be `CyclingPathGazette` if we download the data from the gov website, if we download the data from e-learn, it should be `CyclingPath`. In other words, **always look at the data format** when writing codes.

![](images/paste-F64DEE4A.png){fig-align="center"}
...
The plot function will print an appropriate number of pictures by default. If we want to see all the pictures, we need to set max.plot=n according to the console warning.
...
>
> 3414 is a secret code to transform data into SVY21 format.
