...
Dec 14, 2022 Take-home exercise 2
...
First, we unzip the downloaded data into the folder where the data is stored, and then rename them to "Water_WPdx". Then, we use the filter function to filter and store the data belonging to Nigeria into the variable wp.

```{r}
wp_nga <- read_csv("data/aspatial/Water_WPdx.csv") %>%
...
First, of sf package is used to derive a new field called *Geometry* as shown in the code chunk below.

```{r}
wp_nga$Geometry = st_as_sfc(wp_nga$`New Georeferenced Column`)
```

Now when we open *wp_nga* data frame and scroll to the last field now, a new field called *Geometry* has been added.

Next, will be used to convert the tibble data frame into sf data frame.`st_sf()`
...
We will use the duplicated function to check all shapeName which have duplicates.
...
Notice that a new field called *shapeName* has been added into *wp_sf* sf data frame.
...
In the code chunk below, *freq()* of **funModeling** package is used to display the distribution of fields in .
...
nga_sf_var <- nga_sf %>%
  st_drop_geometry() %>%
  select("functional","nonfunctional", "pct_functional", "pct_nonfunctional", "pct_handpump","pct_usage1k","pct_usageL1k", "pct_ruralWP")
...
In this exercise, if correlation coefficient value is greater than 0.85 then the variables are said to be highly correlated. The correlation plot above shows that and are highly correlated. This suggest that and should be used in the cluster analysis instead of three variables.
...
cluster_vars <- nga_sf_var %>%
  select("pct_functional", "pct_nonfunctional", "pct_handpump", "pct_usageL1k", "pct_ruralWP")
head(cluster_vars,10)
```

```{r}
cluster_vars.cor = cor(cluster_vars[,1:5])
corrplot.mixed(cluster_vars.cor,
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
...
Notice that the final clustering variables list does not include variable because it is highly correlated with variable and .
...
```{r}
nga_cluster_var <- select(cluster_vars, c(1:5))
...
-   *cbind()* is used to append *groups* matrix onto shan_sf to produce an output simple feature object called ; and`nga_sf_cluster`
...
![](images/17eb570f6627b0eb4590c480457f77b.png)

The parallel coordinate plot above reveals that in Cluster 4 tend to own the highest percentage of non-functional water point which matched with choropleth map on . On the other hand, Cluster 1 tends to own the lowest of percentage of rural water points which indicated the regions in cluster 1 should have majority of urban areas.`pct_nonfunctional`

We can also compute the summary statistics such as mean, median, sd, etc to complement the visual interpretation.

In the code chunk below, and of dplyr are used to derive mean values of the clustering variables.`group_by()summarise()`
...
First, we need to convert into SpatialPolygonsDataFrame. This is because SKATER function only support **sp** objects such as SpatialPolygonDataFrame.`nga_sf`
...
![](https://master--hanshuminisss624.netlify.app/take-home_ex/take-home_ex2/Take-home_Ex2_files/figure-html/unnamed-chunk-80-1.png)
...
ClustGeo package is an R package specially designed to support the need of performing spatially constrained cluster analysis. More specifically, it provides a Ward-like hierarchical clustering algorithm called including spatial/geographical constraints.`hclustgeo()`
...
ClustGeo package provides function called to perform a typical Ward-like hierarchical clustering just like you learned in previous section.`hclustgeo()hclust()`
...
![](https://master--hanshuminisss624.netlify.app/take-home_ex/take-home_ex2/Take-home_Ex2_files/figure-html/unnamed-chunk-87-1.png)
...
Notice that is used to convert the data frame into matrix.`as.dist()`

Next, will be used to determine a suitable value for the mixing parameter alpha as shown in the code chunk below.`choicealpha()`
...
Next, is used to derive the cluster objecct.`cutree()`
...
![](https://master--hanshuminisss624.netlify.app/take-home_ex/take-home_ex2/Take-home_Ex2_files/figure-html/unnamed-chunk-95-1.png)

The parallel coordinate plot above reveals that in Cluster 2 tend to own the highest percentage of non-functional water point which the regions are consistent with the choropleth map on . On the other hand, Cluster 1 has lowermost percentage of rural water points.`pct_nonfunctional`
...
The importance of spatial autocorrelation is that it helps define the importance of spatial features in influencing a given object in space, and whether there is a clear relationship between objects and spatial attributes. In this exercise scenario, it is the distribution of water resources in Nigeria, a poor country. The use of LISA, Moran's I and Gi maps allowed us to delve deeper into the geospatial relationships of the various regions of the country, not only clustering similar objects with other similar objects, but also speaking of the degree of relatedness or similarity. It helps to find hidden patterns and relationships. It has many applications in ecology and demography.

From the analysis, we can clearly see that using hierarchical clustering with LGA boundaries and Wand-like clustering is more fragmented compared to other less fragmented clusters. The SKATER clustering algorithm still provides clearer clusters, but when we need to focus on a specific problem in a specific small area, the SKATER method may not reflect the best results because the clear clusters may miss some small details.
